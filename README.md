...
The honest catch: the motion is a **placeholder fixture chosen by a hash of the prompt, not
generated by any model** — so the system does not yet *understand* what you type, and the
variance is a seeded perturbation rather than a model sampling. Everything above is the
research *instrument*; v1 puts a real model behind it. Repo: **Public**.
